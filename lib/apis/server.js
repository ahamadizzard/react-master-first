...
export const getMoviesNewDB = async () => {
  try {
    const response_n = await api.get("v1/moviesnewdb", {
      // const response = await api.get("moviesnewdb", {
      cache: "no-store",
    }); //fetching the data from the server
    if (response_n.ok) {
    ...
    if (error) {
      ...
      const status = error?.response_n?.status; // HTTP staus code (eg. 404, 500)
      const responseBody = await error?.response_n?.json(); // Parse the response body if possible
      console.log("HTTP Error: ", status, responseBody);
    } else {
      //Handle non-HTTP errors (eg. network errors, timeouts)
      console.log("Unknown error", error);
